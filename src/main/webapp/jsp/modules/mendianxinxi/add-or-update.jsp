...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑门店信息</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>门店信息管理</span></li>
								<li class="breadcrumb-item active"><span>编辑门店信息</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">门店信息信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									<input id="updateId" name="id" type="hidden">
									<div class="form-group col-md-6">
										<label>门店编号</label>
										<input id="mendianbianhao" name="mendianbianhao" class="form-control" value="" readonly>
									</div>
									<div class="form-group col-md-6">
										<label>门店名称</label>
									...
									<div class="form-group col-md-6">
										...
										<input name="jiyangjiage" id="jiyangjiage" name="jiyangjiage" class="form-control" placeholder="寄养价格">
											...
											<div class="form-group  col-md-12" >
												<label>门店详情</label>
												<div><div class="upload"><input id="mendianxiangqingupload" name="file" type="file">选择文件</div>
												<script id="mendianxiangqingEditor" type="text/plain"></script>
												<script type="text/javascript">
												    //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
												   //相见文档配置属于你自己的编译器
												 var mendianxiangqingUe = UE.getEditor('mendianxiangqingEditor', {
													toolbars: [
														[
															'undo', //撤销
															'bold', //加粗
												...
												<input type="hidden" name="mendianxiangqing" id="mendianxiangqing-input">
												</div>
											</div>	
	...
	<script>
		...
		function mendianbianhaoUuid(){
			if($("#mendianbianhao").val() == null || $("#mendianbianhao").val() == "null" || $("#mendianbianhao").val() == ""){
				$("#mendianbianhao").attr("value",Math.round(new Date().getTime()/1000));
			}
		}				
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "mendianxinxi/info/" + id,
		...
		function upload(){
			...
			$('#mendianxiangqingupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('mendianxiangqingEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
				...
				if(crossrefid && crossuserid) {
					...
					http("mendianxinxi/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("mendianxinxi/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.mendianxiangqing != null && ruleForm.mendianxiangqing != 'null' && ruleForm.mendianxiangqing != ''){
				var mendianxiangqingMes = '' + ruleForm.mendianxiangqing;
				var mendianxiangqingUeditor = UE.getEditor('mendianxiangqingEditor');
				mendianxiangqingUeditor.ready(function() {
					mendianxiangqingUeditor.setContent(mendianxiangqingMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('mendianxiangqingEditor').hasContents()){
				$('#mendianxiangqing-input').attr('value',UE.getEditor('mendianxiangqingEditor').getContent());
			}
		}
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("mendianxinxi/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dataBind(){
			...
			$("#mendianbianhao").val(ruleForm.mendianbianhao);	
			...
			$("#mendianxiangqing").val(ruleForm.mendianxiangqing);	
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.mendianbianhao != null && res.data.mendianbianhao != ''){

						$("#mendianbianhao").val(res.data.mendianbianhao);
						$("#mendianbianhao").attr('readonly','readonly');
					}
					...
					if(res.data != null && res.data != '' && res.data.mendianxiangqing != null && res.data.mendianxiangqing != ''){

						$("#mendianxiangqing").val(res.data.mendianxiangqing);
                        var mendianxiangqingMes = '' + res.data.mendianxiangqing;
                        var mendianxiangqingUeditor = UE.getEditor('mendianxiangqingEditor');
                        mendianxiangqingUeditor.ready(function() {
                            mendianxiangqingUeditor.setContent(mendianxiangqingMes);
                        });
