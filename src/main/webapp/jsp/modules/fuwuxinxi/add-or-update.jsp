...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑服务信息</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>服务信息管理</span></li>
								<li class="breadcrumb-item active"><span>编辑服务信息</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">服务信息信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									<input id="updateId" name="id" type="hidden">
									<div class="form-group col-md-6">
										<label>服务编号</label>
										<input id="fuwubianhao" name="fuwubianhao" class="form-control" value="" readonly>
									</div>
									<div class="form-group col-md-6">
										<label>服务项目</label>
										<input name="fuwuxiangmu" id="fuwuxiangmu" name="fuwuxiangmu" class="form-control" placeholder="服务项目">
									</div>
									<div class="form-group col-md-6">
										<label>服务类型</label>
									...
									<div class="form-group col-md-6">
										<label>项目等级</label>
										<select id="xiangmudengjiSelect" name="xiangmudengji" class="form-control">
												<option value=""></option>
												<option class="xiangmudengjiOption" value="中等">
													中等
												</option>
												<option class="xiangmudengjiOption" value="中高等">
													中高等
												</option>
												<option class="xiangmudengjiOption" value="高等">
													高等
												</option>
										</select>	
									</div>	
									<div class="form-group col-md-6">
										<label>服务价格</label>
										<input name="fuwujiage" id="fuwujiage" name="fuwujiage" class="form-control" placeholder="服务价格">
									</div>
									<div class="form-group col-md-6">
										<label>服务封面</label>
										<div><img id="fuwufengmianImg" src="" width="100" height="100">
										<div class="upload">选择文件<input name="file" type="file" id="fuwufengmianupload" class="form-control-file"></div>
										<input name="fuwufengmian" id="fuwufengmian-input" type="hidden"></div>
									</div>
									<div class="form-group col-md-6">
										<label>发布日期</label>
										<div id="faburiqiDate" class="input-append date form_datetime">
											<input id="faburiqi-input" name="faburiqi" size="20" type="text" readonly>
											...
											<div class="form-group  col-md-12" >
												...
												<script id="fuwujieshaoEditor" type="text/plain"></script>
	...
	<script>
		...
		function fuwubianhaoUuid(){
			if($("#fuwubianhao").val() == null || $("#fuwubianhao").val() == "null" || $("#fuwubianhao").val() == ""){
				$("#fuwubianhao").attr("value",Math.round(new Date().getTime()/1000));
			}
		}				
		...
		// 文件上传
		function upload(){
			$('#fuwufengmianupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {				
					document.getElementById('fuwufengmian-input').setAttribute('value',"upload/"+data.result.file);
					if(document.getElementById('fuwufengmianFileName') != null){
						document.getElementById('fuwufengmianFileName').innerHTML = "上传成功!";
					}				
					$("#fuwufengmianImg").attr("src",baseUrl+"upload/"+data.result.file);								
				}
			});
				...
				if(crossrefid && crossuserid) {
					...
					http("fuwuxinxi/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("fuwuxinxi/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		function getContent(){
			if(UE.getEditor('fuwujieshaoEditor').hasContents()){
				$('#fuwujieshao-input').attr('value',UE.getEditor('fuwujieshaoEditor').getContent());
			}
		}
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("fuwuxinxi/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dataBind(){
			...
			$("#fuwubianhao").val(ruleForm.fuwubianhao);	
			$("#fuwuxiangmu").val(ruleForm.fuwuxiangmu);	
			...
			var xiangmudengjiOptions = document.getElementsByClassName("xiangmudengjiOption");
			for(var xiangmudengjiCount = 0; xiangmudengjiCount < xiangmudengjiOptions.length;xiangmudengjiCount++){
				if(xiangmudengjiOptions[xiangmudengjiCount].getAttribute('value') == ruleForm.xiangmudengji){
					xiangmudengjiOptions[xiangmudengjiCount].setAttribute('selected','selected');
				}
			}
			$("#fuwujiage").val(ruleForm.fuwujiage);	
			$("#fuwufengmian-input").val(ruleForm.fuwufengmian);
			$("#faburiqi-input").val(ruleForm.faburiqi);
			$("#fuwujieshao").val(ruleForm.fuwujieshao);	
		}		
		//图片显示
		function showImg(){
			var fuwufengmianFileName = "\"" + ruleForm.fuwufengmian + "\"";
			$("#fuwufengmianImg").attr("src",baseUrl+ruleForm.fuwufengmian);
		}		
		//跨表
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.fuwubianhao != null && res.data.fuwubianhao != ''){

						$("#fuwubianhao").val(res.data.fuwubianhao);
						$("#fuwubianhao").attr('readonly','readonly');
					}
					if(res.data != null && res.data != '' && res.data.fuwuxiangmu != null && res.data.fuwuxiangmu != ''){

						$("#fuwuxiangmu").val(res.data.fuwuxiangmu);
						$("#fuwuxiangmu").attr('readonly','readonly');
					}
					...
					if(res.data != null && res.data != '' && res.data.xiangmudengji != null && res.data.xiangmudengji != ''){

						var xiangmudengjiOptions = document.getElementsByClassName("xiangmudengjiOption");
						for(var xiangmudengjiCount = 0; xiangmudengjiCount < xiangmudengjiOptions.length;xiangmudengjiCount++){
							if(xiangmudengjiOptions[xiangmudengjiCount].getAttribute('value') == res.data.xiangmudengji){
								xiangmudengjiOptions[xiangmudengjiCount].setAttribute('selected','selected');
							}
						}
					}
					if(res.data != null && res.data != '' && res.data.fuwujiage != null && res.data.fuwujiage != ''){
						...
						$("#fuwujiage").attr('readonly','readonly');
					}
					if(res.data != null && res.data != '' && res.data.fuwufengmian != null && res.data.fuwufengmian != ''){

						$("#fuwufengmianImg").attr("src",baseUrl+res.data.fuwufengmian);
						$("#fuwufengmian-input").val(res.data.fuwufengmian);
					}
					...
					if(res.data != null && res.data != '' && res.data.fuwujieshao != null && res.data.fuwujieshao != ''){
                        ...
                        var fuwujieshaoMes = '' + res.data.fuwujieshao;
                        var fuwujieshaoUeditor = UE.getEditor('fuwujieshaoEditor');
                        fuwujieshaoUeditor.ready(function() {
                            fuwujieshaoUeditor.setContent(fuwujieshaoMes);
                        });
